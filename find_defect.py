...
def find_defect(video_dir, output_dir):
    print("Taking Video Input : %s" % video_dir)
    print("Result Images : %s" % output_dir)
    output_dir = output_dir + "/defect/"
    start_time = time.time()
    dt = datetime.datetime.now()
    logname = 'defects.txt'
    with open(output_dir + logname, 'w') as f:
        f.write(str(dt.year))
    ...
    while vid.isOpened():
        ...
        if idx % 64 == 0:
            ...
            save_image((vid_tensor[:, :, key_index, :, :].squeeze() + 1) / 2,
                       output_dir + 'defectName' + str(idx) + '.png')

            # TODO : label 2 defect name

            uf = np.empty((64, frameHeight, frameWidth, 3), dtype= np.float32)

            with open(output_dir + logname, 'a') as logfile:
